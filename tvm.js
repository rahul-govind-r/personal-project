const cowinApiTVM = async () => {
    var d = new Date();
    var date = d.getDate() + '-' + (d.getMonth() + 1) + '-' + d.getFullYear()
    const response = await fetch('https://cdn-api.co-vin.in/api/v2/appointment/sessions/public/calendarByDistrict?district_id=296&date=' + date)
    ...
    if (response.status !== 200) 
    {
        ...
        await sendtoTelegramTVM()
        window.location.reload()
    }
    return myJson.centers
}

const checkStatusTVM = async () => {
    var cowinData = await cowinApiTVM()
    if (cowinData.length > 0)
    {
        checkAvailabilityTVM(cowinData)
    }
}

const checkAvailabilityTVM = async (cowinData) => {
...
async function sendtoTelegramTVM () 
{
